...
def prompt(variant):
    ...
    if variant=="D_codes_defs":
        return ("For each line in the input, classify the question by the type of its answer into exactly one code:\n"+
         ...
         "\nOutput exactly one line per input line, in order, formatted '<line number>: <code>'. No other text.")
def call(p,chunk):
    body={"model":"/models/Muse-Glimmer-30B-UD-Q5_K_M.gguf","temperature":0,"max_tokens":6000,
...
out={}
for v in ([] if len(sys.argv)>2 else ["A_plain","B_defs","C_codes","D_codes_defs"]):
    p=prompt(v); conf=collections.Counter(); toks=0; secs=0; nlab=0
    for chunk in CHUNKS:
